...
save(train, test, file = "rds/splits.RData")
```
```{r, echo=FALSE}
load("rds/splits.RData")
```
...
mod_lasso <- train(salary~.-1-X, data = train, method = "glmnet", trControl = train_control, tuneGrid = param_grid)
```
...
```
The r squared of the best tuned model is `r max(mod_lasso$results$Rsquared)`.  
...
```
The R squared is `r max(mod_elaNet$results$Rsquared)`
```{r}
...
The r squared of the model is `r rsq`.  
```{r}
...
mod_rf <- randomForest(salary~., data = train[,-c(1)], ntree = 5000,maxnodes = 100, trControl = trainControl(method = "cv", number = 10), tuneGrid = tunegrid)
```
...
```
The train root mean squared error is `r rmse(train$salary, predict(mod_rf))` while the testing (out of bag) root mean squared error is `r rmse(test$salary, predict(mod_rf, test))`. 
```{r}
...
```
The train root mean squared error is `r rmse(train$salary, predict(mod_rft))` while the testing (out of bag) root mean squared error is `r rmse(test$salary, predict(mod_rft, test))`. 
...
for (i in 1:length(pca.var.per))
{
  ...
  {
    print(
      ggplot(data.frame(score = pca$rotation[,i], var = names(pca$rotation[,i])), aes(x = reorder(var, abs(score)), y = abs(score), fill = ifelse(loading_score > 0, "positive", "negative")))+
      geom_bar(stat = "identity")+
      theme_minimal()+
